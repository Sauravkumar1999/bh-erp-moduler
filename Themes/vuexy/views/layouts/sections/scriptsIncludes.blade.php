...
@endphp
<!-- laravel style -->
<script src="{{ asset('vendor/vuexy/js/helpers.js') }}"></script>
    ...
    pathResolver: function(path) {
      var resolvedPaths = {
        ...
        @foreach (['core'] as $name)
          '{{ $name }}.css': '{{ asset("vendor/vuexyvendor/css{$configData['rtlSupport']}/{$name}.css") }}',
          '{{ $name }}-dark.css': '{{ asset("vendor/vuexyvendor/css{$configData['rtlSupport']}/{$name}-dark.css") }}',
        @endforeach

        // Themes
        @foreach (['default', 'bordered', 'semi-dark'] as $name)
          'theme-{{ $name }}.css': '{{ asset("vendor/vuexyvendor/css{$configData['rtlSupport']}/theme-{$name}.css") }}',
          'theme-{{ $name }}-dark.css':
          '{{ asset("vendor/vuexyvendor/css{$configData['rtlSupport']}/theme-{$name}-dark.css") }}',
        @endforeach
      }
