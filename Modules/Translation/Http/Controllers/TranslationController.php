...
class TranslationController extends Controller
{
    ...
    {
        if (TranslationLanguage::where('slug', $lang)->doesntExist()) {
            return redirect()->back();
        }

        $language = TranslationLanguage::where('slug', $lang)->first();

        $title = $language->lang_name . ' ' . trans_choice('translation::translation.translations', 2);

        $breadcrumbs = [
            ['name' => 'Home', 'url' => route('dashboard')],
            ['name' => trans('translation::translation.translation-languages'), 'url' => route('admin.translation-languages.index')],
            ['name' => $title, 'url' => '#']
        ];
